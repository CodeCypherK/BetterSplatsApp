...
class SessionReader {
 public:
  ...
  // Optional exclusion mask for this frame: <owning session>/masks/NNNNNN.png,
  // non-zero where a pixel may be used (docs/FORMATS.md, "Derived layer").
  //
  // Beside frames/ rather than inside it. A mask is DERIVED — regenerated by
  // whatever segmentation model is current, thrown away and remade — and
  // frames/ is the immutable RAW layer, whose whole value is that a byte
  // comparison after processing still passes. The path resolves through the
  // same chain-owner map as FramePath, so a mask for a parent session's frame
  // is looked for in the parent, where the image it belongs to lives.
  //
  // Returns the path whether or not the file exists; callers test by reading.
  std::string MaskPath(uint32_t frame_id) const;
